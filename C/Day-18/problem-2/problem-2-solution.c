...
void find_blocking_byte(FILE *file, int grid[GRID_SIZE][GRID_SIZE]) {
    int x, y;
    int i = 0;
    ...
    while (fscanf(file, "%d,%d", &x, &y) == 2) {
        ...
        // After each byte, check if the path to the exit is blocked
        if (bfs(grid) == -1) {
            printf("The coordinates of the first byte that blocks the path are: %d,%d\n", x, y);
            return;
        }
    }

    printf("The path to the exit was never blocked by the given bytes.\n");
}
...
int main() {
    ...
    // Find the shortest path using BFS for Part 1
    int result = bfs(grid);
    ...
    // Rewind the file to read all bytes for Part 2
    rewind(file);

    // Reset the grid
    memset(grid, 0, sizeof(grid));

    // Find the first byte that blocks the path for Part 2
    find_blocking_byte(file, grid);
