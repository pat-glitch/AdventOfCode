...
void resize_grid(char grid[GRID_SIZE][GRID_SIZE], int* rows, int* cols) {
    char new_grid[GRID_SIZE][GRID_SIZE] = {0};
    int new_rows = *rows;
    int new_cols = *cols * 2;
        ...
        if (ny >= 0 && ny < rows && nx >= 0 && nx < cols) {
            if (grid[ny][nx] == '.') {
                ...
                *posX = nx;
                ...
                int nny = ny + s->dirs[move].dy;
                int nnx = nx + s->dirs[move].dx;
                if (nny >= 0 && nny < rows && nnx >= 0 && nnx < cols && grid[nny][nnx] == '.') {
                    grid[nny][nnx] = 'O';
                    grid[ny][nx] = '.';
                    *posY = ny;
                    *posX = nx;
...
int part2(Solution* s, char grid[GRID_SIZE][GRID_SIZE], int rows, int cols, char* moves) {
    resize_grid(grid, &rows, &cols);

    int posY, posX;
    get_robot_pos(grid, rows, cols, &posY, &posX);
    grid[posY][posX] = '.';  // Clear the robot's initial position
    move_robot(s, grid, &posY, &posX, moves, rows, cols);
    return calculate_sum(grid, rows, cols, '[');
}
